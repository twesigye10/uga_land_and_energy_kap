...
df_tool_data <- readxl::read_excel("../inputs/merged_data.xlsx") %>% 
  ...
  mutate(uuid = `_uuid`,
         ...
         refugee_settlement = ifelse(refugee_settlement == "adjumani", 
                                     str_to_lower(refugee_settlement_zone), refugee_settlement),
    ...
    filter(!str_detect(string = point_number, pattern = fixed('test', ignore_case = TRUE)) & 
               !uuid %in% c("bbf72f6e-2248-44de-9666-75304e050242")) 
...
df_cl_log %>% 
  ...
  group_by(district_name, location, enumerator_id) %>% 
